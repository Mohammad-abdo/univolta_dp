...
router.get("/dashboard/stats", requirePartner, async (req, res, next) => {
    try {
...
router.get("/applications", requirePartner, async (req, res, next) => {
    try {
        const universityId = req.partnerUniversityId;
...
router.get("/applications/:id", requirePartner, async (req, res, next) => {
    try {
        const { id } = req.params;
        const universityId = req.partnerUniversityId;
...
router.post("/applications", requirePartner, async (req, res, next) => {
    try {
        const universityId = req.partnerUniversityId;
        const { fullName, email, phone, personalAddress, dateOfBirth, academicQualification, identityNumber, country, programId, additionalServices, additionalNotes, applicationFee = 100, } = req.body;
        ...
        await createOperationAlert("create", "applications", application.fullName, application.id, req.user?.id, universityId, {
...
router.patch("/applications/:id/status", requirePartner, async (req, res, next) => {
    try {
        const { id } = req.params;
        const universityId = req.partnerUniversityId;
        ...
        if (applicationWithDetails) {
            await createStatusChangeAlert("applications", applicationWithDetails.fullName, applicationWithDetails.id, previousStatus, status, req.user?.id, universityId, {
...
router.put("/applications/:id", requirePartner, async (req, res, next) => {
    try {
        const { id } = req.params;
        const universityId = req.partnerUniversityId;
        const { fullName, email, phone, personalAddress, dateOfBirth, academicQualification, identityNumber, country, additionalNotes, notes, } = req.body;
        ...
        await createOperationAlert("update", "applications", updated.fullName, updated.id, req.user?.id, universityId, {
            email: updated.email,
...
router.patch("/applications/:id/block", requirePartner, async (req, res, next) => {
    try {
        const { id } = req.params;
        const universityId = req.partnerUniversityId;
...
router.delete("/applications/:id", requirePartner, async (req, res, next) => {
    try {
        const { id } = req.params;
        const universityId = req.partnerUniversityId;
        // Verify application belongs to partner's university
        ...
        await createOperationAlert("delete", "applications", application.fullName, application.id, req.user?.id, universityId, {
            email: application.email,
            universityName: application.university?.name,
            programName: application.program?.name,
        });
...
router.get("/payments", requirePartner, async (req, res, next) => {
    try {
        const universityId = req.partnerUniversityId;
        const { status, search, page = "1", limit = "10" } = req.query;
        const where = {
...
router.get("/programs", requirePartner, async (req, res, next) => {
    try {
        const universityId = req.partnerUniversityId;
...
router.get("/programs/:id", requirePartner, async (req, res, next) => {
    try {
        const { id } = req.params;
        const universityId = req.partnerUniversityId;
        const program = await prisma.program.findFirst({
...
router.post("/programs", requirePartner, async (req, res, next) => {
    try {
        const universityId = req.partnerUniversityId;
        const { name, slug, degree, duration, language, tuition, tuitionNotes, studyYear, lastApplicationDate, classSchedule, coreSubjects, about, department, programImages, } = req.body;
        if (!name || !slug) {
        ...
        await createOperationAlert("create", "programs", program.name, program.id, req.user?.id, universityId, { degree: program.degree, universityName: program.university.name });
...
// Update program
router.put("/programs/:id", requirePartner, async (req, res, next) => {
    try {
        const { id } = req.params;
        const universityId = req.partnerUniversityId;
        // Verify program belongs to partner's university
        ...
        await createOperationAlert("update", "programs", updated.name, updated.id, req.user?.id, universityId, { degree: updated.degree, universityName: updated.university.name });
        res.json(updated);
    }
    catch (error) {
        next(error);
...
// Delete program
router.delete("/programs/:id", requirePartner, async (req, res, next) => {
    try {
        const { id } = req.params;
        const universityId = req.partnerUniversityId;
        // Verify program belongs to partner's university
        ...
        await createOperationAlert("delete", "programs", program.name, program.id, req.user?.id, universityId, { degree: program.degree, universityName: program.university.name });
...
// Get reports data
router.get("/reports", requirePartner, async (req, res, next) => {
    try {
        const universityId = req.partnerUniversityId;
        ...
        applicationsByStatus.forEach((item) => {
            applicationsByStatusFormatted[item.status] = item._count;
        });
        ...
        paymentsByStatus.forEach((item) => {
            ...
            if (status in paymentsByStatusFormatted) {
                paymentsByStatusFormatted[status] = item._count;
            }
        });
        res.json({
            totalApplications,
...
// Export reports
router.get("/reports/export", requirePartner, async (req, res, next) => {
    try {
        const universityId = req.partnerUniversityId;
        ...
        if (format === "csv") {
            ...
            applications.forEach((app) => {
                csvRows.push(`"${app.fullName}","${app.email}","${app.program?.name || "N/A"}","${app.status}","${app.applicationFee || 0}","${app.payment?.paymentStatus || "N/A"}","${new Date(app.createdAt).toLocaleString()}"`);
            });
            csvRows.push("");
            // Payments
            ...
            payments.forEach((payment) => {
                csvRows.push(`"${payment.application.fullName}","${payment.application.email}","${payment.application.program?.name || "N/A"}","${payment.amount}","${payment.paymentStatus}","${payment.paymentMethod}","${payment.transactionId || "N/A"}","${new Date(payment.createdAt).toLocaleString()}"`);
            });
            csvRows.push("");
            // Programs
            ...
            programs.forEach((program) => {
                csvRows.push(`"${program.name}","${program.degree || "N/A"}","${program.duration || "N/A"}","${program.language || "N/A"}","${program.tuition || "N/A"}","${program._count.applications}"`);
            });
...
router.get("/university", requirePartner, async (req, res, next) => {
    try {
        const universityId = req.partnerUniversityId;
...
router.put("/university", requirePartner, async (req, res, next) => {
    try {
        const universityId = req.partnerUniversityId;
        const data = req.body;
        ...
        await createOperationAlert("update", "universities", university.name, university.id, req.user?.id, universityId, { country: university.country, city: university.city });
        res.json(university);
    }
    catch (error) {
...
//# sourceMappingURL=partner.router.js.map