...
router.get("/", async (_req, res, next) => {
    try {
        ...
        res.json(universities.map((university) => ({
            ...university,
...
router.post("/", requirePermission("universities", "create"), async (req, res, next) => {
    try {
        ...
        await createOperationAlert("create", "universities", university.name, university.id, req.user?.id, university.id, { country: university.country, city: university.city });
...
router.put("/:id", requirePermission("universities", "update"), async (req, res, next) => {
    try {
        ...
        await createOperationAlert("update", "universities", university.name, university.id, req.user?.id, university.id, { country: university.country, city: university.city });
        res.json(university);
    }
    catch (error) {
...
router.delete("/:id", requirePermission("universities", "delete"), async (req, res, next) => {
    try {
        ...
        await createOperationAlert("delete", "universities", university.name, university.id, req.user?.id, university.id);
        res.status(204).send();
    }
    catch (error) {
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025") {
