...
import { createProgramSchema, listProgramsQuerySchema, updateProgramSchema, } from "./program.validator.js";
...
router.post("/", requirePermission("programs", "create"), async (req, res, next) => {
    try {
        ...
        await createOperationAlert("create", "programs", program.name, program.id, req.user?.id, program.universityId, { degree: program.degree, universityName: program.university.name });
...
router.put("/:id", requirePermission("programs", "update"), async (req, res, next) => {
    try {
        ...
        await createOperationAlert("update", "programs", program.name, program.id, req.user?.id, program.universityId, { degree: program.degree, universityName: program.university.name });
        res.json(program);
    }
    catch (error) {
...
router.delete("/:id", requirePermission("programs", "delete"), async (req, res, next) => {
    try {
        ...
        await createOperationAlert("delete", "programs", program.name, program.id, req.user?.id, program.universityId, { universityName: program.university.name });
        res.status(204).send();
    }
    catch (error) {
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025") {
...
//# sourceMappingURL=program.router.js.map