...
async function main() {
    ...
    for (const uniData of universitiesData) {
        ...
        for (const progData of universityPrograms) {
            ...
            const department = createdDepartments.find(d => progData.department?.toLowerCase().includes(d.name.toLowerCase().split(" ")[1]?.toLowerCase() || "")) || createdDepartments[0];
            const degree = createdDegrees.find(d => progData.degree?.toLowerCase().includes(d.name.toLowerCase().split(" ")[0]?.toLowerCase() || "")) || createdDegrees[0];
